...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="person[not(floruit[@period='1'])]"/>
    <xsl:template match="person[floruit[@period='1']]">
        ...
        <xsl:if test="floruit[not(location/@corresp)]">
            <xsl:apply-templates select="floruit[@period='1']"/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="floruit[@period='1']">
        <Placemark>
            <name>
